...
// check if neigbor lane is safe to change to
bool check_lane(vector<vector<double>> sensor_fusion, double car_s, int lane, int prev_size) {
  // find ref_v to use
  for (int i = 0; i < sensor_fusion.size(); i++) {
    ...
    if (d > (2 + 4 * lane - 2) && d < (2 + 4 * lane + 2)) {
      ...
      if (check_car_s > (car_s - 10) && (check_car_s - car_s) < 10) {
        return false;
      }
    }
  }
...
int main() {
  ...
  h.onMessage([&ref_lane, &ref_speed, &max_speed, &map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy](
...
//          cout << "prev_size: " << prev_size << endl;
          ...
          // behavior planning
          // make use of sensor fusion information to avoid collison
          if (prev_size > 0) {
            car_s = end_path_s;
          }
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            if (d > (2 + 4 * ref_lane - 2) && d < (2 + 4 * ref_lane + 2)) {
              ...
              if (check_car_s > car_s && (check_car_s - car_s) < 30) {
                ...
                too_close = true;
                if (too_close && (ref_speed > check_speed * 2.24 || (check_car_s - car_s) < 20)) {
                  ref_speed -= 0.224; // mph
                }
                if (check_speed <= max_speed -3) { // less than 45mph, we want to pass it
                  // try lane on the left
                  if (ref_lane != 0 && check_lane(sensor_fusion, car_s, ref_lane - 1, prev_size)) {
                    ref_lane--;
                  } else if (ref_lane != 2 && check_lane(sensor_fusion, car_s, ref_lane + 1, prev_size)) {
                    ref_lane++;
                  }
                }
              }
          ...
          // given ref speed and ref lane
          // create a list of widely spaced (x, y) anchor points, then use spline to interoplate to get a smooth path
          vector<double> ptsx;
          vector<double> ptsy;

          // reference of car position
          double ref_x = car_x;
          double ref_y = car_y;
          ...
          if (prev_size == 0) {
            ...
            double prev_car_x = car_x - cos(car_yaw); // x - 1 * cos(yaw)
            double prev_car_y = car_y - sin(car_yaw); // y - 1 * sin(yaw)
